...
import { saveConfig as _saveConfig, SaveConfigArgs } from "../server-actions";
...
const DesignConfigurator = ({ configId, imgUrl, imageDimensions }: DesignConfiguratorProps) => {
  ...
  const { mutate: saveFile } = trpc.saveFile.useMutation({
    ...
    onSuccess: (data: any) => {
      if (data) {
        router.push(`/configure/preview?id=${data.configId}`);
      }
    },
    onError: () => {
      return toast({
  ...
  const { mutate: uploadFile } = useMutation({
    ...
    onSuccess: (data) => {
      if (data) {
        saveFile({ url: data.url, configId });
      }
    },
  });

  const { mutate: saveConfig, isLoading } = useMutation({
